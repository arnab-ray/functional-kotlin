...
fun <A> isSorted(aa: List<A>, order: (A, A) -> Boolean): Boolean {
    tailrec fun loop(aa: List<A>, order: (A, A) -> Boolean): Boolean {
        return when {
            aa.isEmpty() || aa.tail.isEmpty() || aa.tail.head == null -> true
            order(aa.head, aa.tail.head) -> loop(aa.tail, order)
            else -> false
        }
    }

    return loop(aa, order)
}
